...
namespace f3b_store.services
{
    public class EmailService
    {
            ...
            using (MailMessage mm = new MailMessage("store@f3b.com", email))
            {
                ...
                foreach(var item in items)
                {
                    ...
                    while(iteration < item.Amount) {
                        string itemName = DBOperations.selectQuery("SELECT name FROM product_info WHERE id LIKE '" + item.ProductId + "'").Rows[0]["name"].ToString();
        ...
        internal static void UserRegisterConfirmation(string email, string username)
        {
            ...
            {
                var id = DBOperations.selectQuery("SELECT id FROM users WHERE username LIKE '" + username + "'");
                var idTrue = "";
                if (id.Rows.Count == 1)
                ...
                body += "<br /><a href = 'http://localhost:54291/Verification.aspx/?ActivationCode=" + idTrue + "'>Aktywuj konto</a>";
                mm.Body = body;
                mm.IsBodyHtml = true;
                SmtpClient smtp = new SmtpClient();
                smtp.Host = "smtp.gmail.com";
